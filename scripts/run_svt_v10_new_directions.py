"""
SVT-v10: Conflict-Augmented Training + Contrastive Trajectory Identity

Two new directions:
F: Conflict-augmented training (force model to experience conflict)
G: Contrastive trajectory identity (learn trajectory representations)

Key question: Can conflict augmentation prevent FeatureOnly collapse?
"""
...
OUTPUT_DIR = "results/svt_v10_new_directions"
EPOCHS = 30
SEEDS = [0, 42, 123]
...
def run_conflict_augmented_experiment(num_objects, feature_dim, seed, epochs=EPOCHS):
    ...
    for p_conflict in [0.0, 0.3, 0.5]:
        for p_drop in [0.0, 0.1]:
            if p_conflict == 0.0 and p_drop == 0.0:
                tag = "no_aug"
            elif p_conflict > 0 and p_drop == 0.0:
                tag = f"conf{p_conflict}"
            elif p_conflict > 0 and p_drop > 0:
                tag = f"conf{p_conflict}_drop{p_drop}"
            else:
                tag = f"drop{p_drop}"

            print(f"  Training CA-LearnedObjectFile ({tag})...")
            model = ConflictAugmentedLearnedObjectFile(
                ...
                p_conflict=p_conflict, p_feature_drop=p_drop)
            ...
            for epoch in range(epochs):
                ...
                for batch_idx in range(n_batches):
                    ...
                    loss, comb_l, feat_l, traj_l = model.compute_loss(
            ...
            for method in ["combined", "feature", "trajectory"]:
                ...
                if len(no_swap_idx) > 0:
                    ...
                    pred_id_c = model.predict_identity(
                ...
                all_results.append({
                    "mechanism": f"CA-LOF_{tag}/{method}",
            ...
            combined_r = [r for r in all_results if r["mechanism"] == f"CA-LOF_{tag}/combined" and r["seed"] == seed]
            if combined_r:
                r = combined_r[-1]
                print(f"    {tag}/combined: swap={r['swap_only']} conflict_a={r['conflict_a']}")

    print(f"  Training CFObjectFile (rule-based baseline)...")
...
def main():
    ...
    print("=" * 70)
    print("SVT-v10: Conflict-Augmented Training + New Directions")
    print("=" * 70)
    ...
    for seed in SEEDS:
        print(f"\n--- Seed {seed} ---")
        results = run_conflict_augmented_experiment(2, 2, seed)
        all_results.extend(results)

    print("\n=== 3 Objects + 16-dim Continuous Features ===")
    for seed in SEEDS:
        print(f"\n--- Seed {seed} ---")
        results = run_conflict_augmented_experiment(3, 16, seed)
        all_results.extend(results)

    save_csv(all_results, "v10_results.csv",
    ...
    print("\n" + "=" * 70)
    print("CONFLICT AUGMENTATION COMPARISON (mean ± std)")
    print("=" * 70)

    from collections import defaultdict
    grouped = defaultdict(list)
    for r in all_results:
        if "/combined" in r["mechanism"] or r["mechanism"] == "CFObjectFile_rule":
            key = f"{r['mechanism']}_{r['num_objects']}obj"
            grouped[key].append(r)

    print(f"\n{'Mechanism':<50} {'Swap-Only':>12} {'Conflict-A':>12}")
    print("-" * 75)
    ...
    for key, runs in sorted(grouped.items()):
        ...
        print(f"{key:<50} {fmt_stat(swaps):>12} {conflict_str:>12}")

    print("\n" + "=" * 70)
    print("KEY QUESTION: Does conflict augmentation break the trade-off?")
    print("=" * 70)

    for n_obj in [2, 3]:
        print(f"\n  {n_obj} objects:")
        cf_swaps = [float(r["swap_only"]) for r in all_results
                    if r["mechanism"] == "CFObjectFile_rule" and r["num_objects"] == n_obj]
        cf_conflicts = [float(r["conflict_a"]) for r in all_results
                        if r["mechanism"] == "CFObjectFile_rule" and r["num_objects"] == n_obj
        ...
        for tag in ["conf0.3", "conf0.5", "conf0.3_drop0.1", "conf0.5_drop0.1"]:
            ca_swaps = [float(r["swap_only"]) for r in all_results
                        if r["mechanism"] == f"CA-LOF_{tag}/combined" and r["num_objects"] == n_obj]
            ca_conflicts = [float(r["conflict_a"]) for r in all_results
                            if r["mechanism"] == f"CA-LOF_{tag}/combined" and r["num_objects"] == n_obj
                            and r.get("conflict_a", "nan") != "nan"]

            if ca_swaps and ca_conflicts:
                mean_s = np.mean(ca_swaps)
                mean_c = np.mean(ca_conflicts)
                product = mean_s * mean_c
                if product > best_swap * best_conflict:
                    best_swap = mean_s
                    best_conflict = mean_c
                    best_tag = tag

        cf_s = np.mean(cf_swaps) if cf_swaps else 0
        cf_c = np.mean(cf_conflicts) if cf_conflicts else 0

        print(f"    Best CA-LOF ({best_tag}): swap={best_swap:.3f}, conflict_a={best_conflict:.3f}, product={best_swap*best_conflict:.3f}")
        print(f"    CFObjectFile:           swap={cf_s:.3f}, conflict_a={cf_c:.3f}, product={cf_s*cf_c:.3f}")
        print(f"    CA-LOF product > CF:    {'YES!' if best_swap*best_conflict > cf_s*cf_c else 'NO'}")
